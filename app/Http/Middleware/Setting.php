...
class Setting
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (tenant('domains') == null) {
            config([
                'chatify.routes.middleware' => env('CHATIFY_ROUTES_MIDDLEWARE', ['api', 'auth', 'Setting'])
            ]);
        } else {
            config([
                'chatify.routes.middleware' => env('CHATIFY_ROUTES_MIDDLEWARE', ['api', 'auth', 'Setting', Middleware\InitializeTenancyByDomain::class])
            ]);
        }
